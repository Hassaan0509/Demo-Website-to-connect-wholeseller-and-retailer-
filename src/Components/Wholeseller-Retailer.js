...
export default function Retailer() {
  ...
  const retailerData = useSelector((state) => state.retailerData);
  const [confirmMessage, setConfirmMessage] = useState(false);
  const [temp, setTemp] = useState(null);
  const dispatch = useDispatch();
  const { retailer__Data } = bindActionCreators(actionCreator, dispatch);
            ...
            <div className="row justify-content-center">
              ...
              <div className="col-md-4 mx-2">
                <button
                  ...
                  onClick={() => {
                    retailer__Data(retailerData.filter((obj) => obj !== temp));
                    setConfirmMessage(false);
      ...
      >
        <div className="bg-success w-100">
          <div className="d-flex align-items-center py-4 text-light">
            ...
            <h1 className="header-item">Retailers</h1>
        ...
        {retailerData.map((element) => {
          return (
            <>
              <div
                className="row fw-semibold fs-6 pt-3 pb-1 retailer__Data position-relative"
                key={element.key}
              >
                <Link
                  to={element.link}
